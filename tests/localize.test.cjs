...
} = load("src/localize/localize.ts");
assert.deepEqual([...requiredTranslationFragments].sort(), [
  "config",
  "lovelace",
]);
  ...
  for (const language of ["en-US", "en-GB", "de", "fr"]) {
    assert.equal(
      localize(key, {
        language,
        localize: (lookup) =>
          lookup === nativeKey ? "Native wording" : undefined,
      }),
      "Native wording",
    );
  }
...
assert.equal(
  localizeSignal("Medium (50%)", { language: "de" }),
  "Mittel (50%)",
);
assert.equal(
  localizeSignal("Medium", {
    language: "de",
    localize: (key) =>
      key === auditedKeys["signal.medium"] ? "HA medium" : undefined,
  }),
  "HA medium",
);
