...
assert.equal(
  sanitizeConfig({ ...original, group_by: "none" }).layout_data,
  undefined,
);
assert.deepEqual(
  sanitizeConfig({ orientation: "pie", layout_data: layout }).layout_data,
  layout,
);
console.log(
  "Configuration sanitization removes redundant guards and rejects stale coordinates.",
);
assert.equal(
