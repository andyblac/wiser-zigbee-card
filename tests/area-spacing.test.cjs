...
assert.deepEqual(
  nodes.slice(0, 3),
  original.slice(0, 3),
  "Hub and repeater area stay anchored",
);
...
assert.equal(
  nodes[3].y - nodes[4].y,
  110,
  "Area header and device move together",
);
assert.ok(
  Math.hypot(nodes[3].x - original[3].x, nodes[3].y - original[3].y) <= 240,
);
...
assert.deepEqual(
  nodes,
  settled,
  "Separated areas do not drift on another pass",
);
...
assert.deepEqual(
  locked,
  original,
  "Unavoidable repeater-area overlap is allowed",
);
console.log(
  "Area spacing reduces overlap with bounded movement and stable anchors.",
);
...
assert.ok(
  measuredNodes[1].x >= 296 || Math.abs(measuredNodes[1].y) >= 136,
