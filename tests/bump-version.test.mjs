...
test('build bumps only dev versions in both package files and preserves tagged releases', () => {
  ...
  try {
    for (const [before, after, skip] of [
      ...
      writeFileSync(join(root, 'package.json'), JSON.stringify({version: before, scripts: {build: 'example'}}));
      writeFileSync(join(root, 'package-lock.json'), JSON.stringify({version: before, packages: {'': {version: before}, dependency: {version: '1.0.0'}}}));
      execFileSync(process.execPath, [script], {cwd: root, env: {...process.env, WISER_SKIP_VERSION_BUMP: skip}});
      ...
      if (before === after) {
        assert.equal(readFileSync(join(root, 'package.json'), 'utf8'), JSON.stringify({version: before, scripts: {build: 'example'}}));
        assert.equal(readFileSync(join(root, 'package-lock.json'), 'utf8'), JSON.stringify({version: before, packages: {'': {version: before}, dependency: {version: '1.0.0'}}}));
      }
    }
