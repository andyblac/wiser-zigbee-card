...
const { buttonConfirmation } = require("./load-ts.cjs")(
  "src/action-confirmation.ts",
);
...
const doc = {
  createElement: make,
  body: { append: (element) => nodes.push(element) },
};
try {
  const button = {
    ownerDocument: doc,
  ...
  assert.equal(
    nodes.length,
    0,
    "No success feedback before the action completes",
  );
  confirm();
  ...
  console.log(
    "Success uses a native tooltip at the captured button position and dismisses cleanly.",
  );
} finally {
