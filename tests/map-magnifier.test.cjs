...
const { MapMagnifier, lensGeometry } = require("./load-ts.cjs")(
  "src/map-magnifier.ts",
);
...
assert.equal(frame, undefined);
console.log(
  "Magnifier tracks pointer, stays inside map, handles retina canvas and native area icons, and yields to dragging/touch.",
);
...
for (const zoom of [2, 3, 4]) {
  ...
  assert.equal(
    new MapMagnifier().zoom,
    zoom,
    "Browser remembers magnification",
  );
...
assert.equal(
  blocked.zoom,
  3,
  "Lens still works with browser storage unavailable",
);
global.localStorage = {
  getItem: (key) => storage.get(key) ?? null,
  setItem: (key, value) => storage.set(key, value),
