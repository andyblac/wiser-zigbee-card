...
(async () => {
  ...
  const raw = {
    views: [
      {
        sections: [
          { cards: [{ type: "vertical-stack", cards: [card, duplicate] }] },
        ],
      },
    ],
  };
  ...
  const result = await saveCardConfig(host, card, {
    show_labels: true,
    map_only: true,
  });
...
(async () => {
  let received;
  const panel = {
    localName: "wiser-zigbee-panel",
    saveZigbeeCardConfig: async (card, config) => {
      received = { card, config };
      return config;
    },
  };
  const host = { getRootNode: () => ({ host: panel }) };
  const result = await saveCardConfig(
    host,
    { type: "custom:wiser-zigbee-card", hub: "first" },
    { show_labels: true },
  );
