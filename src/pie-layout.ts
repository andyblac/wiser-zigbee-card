...
export function arrangePie(
  ...
  const measure = (id: number): { radius: number; orbit: number } => {
    ...
    const childRadius = Math.max(
      0,
      ...descendants.map((child) => measure(child).radius),
    );
    const orbit = descendants.length
      ? Math.max(
          own + childRadius + gap,
          descendants.length > 1
            ? (childRadius + gap / 2) / Math.sin(Math.PI / descendants.length)
            : 0,
        )
      : 0;
    const size = {
      radius: orbit + (descendants.length ? childRadius : own),
      orbit,
    };
    sizes.set(id, size);
    return size;
  ...
  const place = (id: number, x: number, y: number, outward: number) => {
    ...
    const angleOrder = (child: number) => {
      ...
      return (
        (((Math.atan2(p.y - origin.y, p.x - origin.x) - outward) %
          (2 * Math.PI)) +
          2 * Math.PI) %
        (2 * Math.PI)
      );
    ...
    for (const [parentId, siblings] of children) {
      ...
      for (const leafId of siblings.filter((id) => !children.has(id))) {
        ...
        const anchors = siblings
          .filter(
            (id) => children.has(id) && byId.get(id)!.area_id === leaf.area_id,
          )
          .map((id) => nodes.find((node) => node.id === id)!)
          .sort(
            (a, b) =>
              Math.hypot(a.x - leaf.x, a.y - leaf.y) -
              Math.hypot(b.x - leaf.x, b.y - leaf.y),
          );
        let placed = false;
        for (const anchor of anchors) {
          const angle = Math.atan2(anchor.y - parent.y, anchor.x - parent.x);
          const inwardStep = Math.max(
            150,
            (bounds(anchor).right -
              bounds(anchor).left +
              bounds(leaf).right -
          ...
          const offsets = [0, 1, 2, 3].map((column) => ({
            inward: inwardStep + column * 90,
            side: 65,
          }));
          for (let row = 1; row < 5; row++)
            for (const column of [1, 0, 2])
              offsets.push({
                inward: inwardStep + column * 90,
                side: 65 + row * 130,
              });
          for (const offset of offsets) {
            ...
            for (const side of [1, -1]) {
              ...
              const area = {
                left: Math.min(candidate.left, anchorBox.left) - 18,
