...
export function separateAreas(
  nodes: MapNode[],
  repeaterIds: Set<number>,
  ...
  const boxes = (members: MapNode[]) =>
    measuredBounds?.(members) ?? {
      left:
        Math.min(
          ...members.map((n) => n.x - Math.max(40, n.label.length * 4.5)),
        ) - 18,
      right:
        Math.max(
          ...members.map((n) => n.x + Math.max(40, n.label.length * 4.5)),
  ...
  const needsPacking = allBoxes.some((box, i) =>
    allBoxes.slice(i + 1).some((other) => overlap(box, other) > 0),
  );
  const extent = {
    left: Math.min(...allBoxes.map((b) => b.left)),
  ...
  for (let pass = 0; pass < 12; pass++) {
    ...
    for (const members of entries) {
      ...
      const score = (candidate: typeof box) =>
        others.reduce((sum, other) => sum + overlap(candidate, other), 0);
      const compactness = (candidate: Box) => {
        const combined = [...others, candidate];
        return (
          (Math.max(...combined.map((b) => b.right)) -
            Math.min(...combined.map((b) => b.left))) *
          (Math.max(...combined.map((b) => b.bottom)) -
            Math.min(...combined.map((b) => b.top)))
        );
      };
      let best = score(box);
      ...
      for (const other of others.filter(
        (other) => needsPacking || overlap(box, other) > 0,
      )) {
        const candidates = [
        ...
        for (const candidate of candidates) {
          if (
            members.some(
              (node) =>
                Math.hypot(
                  node.x + candidate.x - origins.get(node.id)!.x,
                  node.y + candidate.y - origins.get(node.id)!.y,
                ) > movementLimit,
            )
          )
            continue;
          const shifted = {
            left: box.left + candidate.x,
            right: box.right + candidate.x,
            top: box.top + candidate.y,
            bottom: box.bottom + candidate.y,
          };
          if (
            nodes.some(
              (node) =>
          ...
          if (
            next < best ||
            (needsPacking &&
              Math.abs(next - best) < 0.01 &&
              compact < bestCompactness - 1)
          ) {
            best = next;
