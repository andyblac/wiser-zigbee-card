...
export async function saveCardConfig(
  host: Element,
  original: Config,
  changes: Config,
): Promise<Config> {
  ...
  while (current) {
    if (
      current.localName === "wiser-zigbee-panel" &&
      current.saveZigbeeCardConfig
    ) {
      return current.saveZigbeeCardConfig(
        host,
        sanitizeConfig({ ...original, ...changes }),
      );
    }
  ...
  const visit = (value: any, path: (string | number)[]) => {
    ...
    if (
      ...
      canonical(value) === canonical(original)
    )
  ...
  if (candidates.length !== 1)
    throw new Error("Cannot uniquely identify this card");
  ...
  const finishScrollRestore = preserveDashboardScroll(
    host.ownerDocument?.defaultView ?? null,
  );
  try {
