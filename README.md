...
A compact network map for the Wiser Home Assistant integration. Device images and short room names replace coloured text boxes. Tap a device to see its full name and connection quality; use **Link labels** to show compact percentages, or status when no percentage is available. When labels are disabled, hovering over a link temporarily shows its label. Full signal descriptions remain in device details. Labels use 14 px text with HA tooltip theme colours, reserve space and move along their links to avoid other labels and devices. On very dense maps, labels with no free space remain hidden until there is room; their details remain available by tapping a device. The expandable device list also exposes names and links without using the canvas.
...
Each orientation has separate browser-saved positions. Existing saved positions continue to work in the original Horizontal view. Saved configuration and layout YAML use only `orientation` and `group_by`. Legacy `layout_orientation` and `layout_group_by` guards are removed on save; changing orientation or grouping in the editor clears the previous coordinates.
...
On editable storage dashboards, **Save layout** updates this card’s configuration using HA’s dashboard save mechanism. In the card editor it updates the draft; use the editor’s Save button to apply it. YAML dashboards require copying the exported layout YAML. If the card cannot be identified uniquely or the dashboard save fails, an error is shown.

**Magnifying glass** adds a circular lens (2× by default) that follows the mouse or pen over the map without changing zoom or device positions. Enable it with the magnifying-glass toolbar button or the native editor switch (`magnifier: true`; default off). The lens includes device images, links, labels and native HA area icons, and hides while dragging or using touch. Save layout also saves whether the lens is enabled. Long-press the magnifier button for a native menu with 2×, 3× and 4× magnification; the menu also includes a native vertical lens-size slider to the right of the zoom options (100–360 px, default 180 px). Your browser remembers both choices automatically. Right-click or press Arrow Down while the button is focused to open the same menu.

A saved card configuration takes priority over browser-local layout positions, so the same dashboard arrangement is used on other computers. Browser positions are used only when no compatible layout is configured. Auto refresh still preserves unsaved dragging and the current viewport.
